...
class Expression:
    ...
    def __init__(self, original_expr: str):
        """
        参数:
            original_expr (str): 原始中缀表达式
        """

        self.original_expr = original_expr
        self.tokens = self.lexical_analysis()
        self.rpn_expr = self.generate_rpn()
    ...
    def lexical_analysis(self):
        ...
        """
        tokens = []
        elements = self.original_expr.split()
        # 定义合法的字符集
        valid_tokens = set('0123456789+-*/() ')

        # 检查每一个词法单元
        for token in self.original_expr:
            for char in token:
    ...
    def evaluate(self) -> RationalNumber:
        """计算逆波兰式的值，并返回结果。
        返回:
            RationalNumber: 计算结果
        """
    ...
    def __str__(self):
        ...
        返回:
            str: 原始表达式和逆波兰式
        """
        return f"原始表达式: {self.original_expr}, 逆波兰式: {' '.join(self.rpn_expr)}"
